...
export function FloatingPanel({
  ...
  const shouldShowPromptOnly = state === 'idle' && showPrompt;

  return (
    <div
      className={`polish-floating-panel ${shouldShowPromptOnly ? 'polish-selection-prompt' : ''} ${
        isVisible ? 'polish-visible' : ''
      }`}
      style={{
        position: 'absolute',
        top: `${position.top}px`,
    ...
    >
      {shouldShowPromptOnly ? (
        <button className="polish-selection-btn" onClick={onPrompt}>
          润色选中文本
        </button>
      ) : (
        <>
